...
# max_tokens is capped below the Groq free-tier OTPM limit (1000 output tokens/minute)
# to avoid 429 RateLimitError responses. Raise this if you're on a higher tier.
# timeout/max_retries added so transient network blips (e.g. connection timeouts)
# are retried automatically instead of crashing the whole script.
llm = ChatGroq(model="qwen/qwen3.6-27b", temperature=0, max_tokens=512, timeout=30, max_retries=3)
vision_llm = ChatGroq(model="meta-llama/llama-4-scout-17b-16e-instruct", temperature=0, max_tokens=512, timeout=30, max_retries=3)
...
def search_products(
    ...
    Optionally filter by maximum price and/or organic status (true = organic only,
    false = non-organic only, omit to include both).
...
if __name__ == "__main__":
    print("Starting agent run...", flush=True)
    try:
        result = agent.invoke(
            {
                "messages": [
        ...
        print("Number of messages in result:", len(result["messages"]), flush=True)
        print("Final message object:", repr(result["messages"][-1]), flush=True)
        print("Final content:", result["messages"][-1].content, flush=True)
    except Exception as e:
        print("EXCEPTION DURING AGENT RUN:", repr(e), flush=True)
        raise